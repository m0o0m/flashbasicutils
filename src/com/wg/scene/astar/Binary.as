package com.wg.scene.astar
{
	...
	public class Binary
	{
		...
		private var _dataNodeMap:Dictionary;
		...
		{
			_data = new Array();
			_dataNodeMap = new Dictionary(true);
		...
		public function push(node:Object):void
		{
			...
			if( len > 1 )
			{
				...
				/** 新节点当前父节点所在索引 */
				var parentIndex:int = index * 0.5 - 1;
				...
				while(compareTwoNodes(node, _data[parentIndex]) )
				{
					...
					validNodeIndex = parentIndex;
					
					index *= 0.5;
					parentIndex = index * 0.5 - 1;
				}
			}
			
			_dataNodeMap[node] = true;
		}
		...
		public function has(node:Object):Boolean
		{
			return _dataNodeMap[node] !== undefined;
		}
