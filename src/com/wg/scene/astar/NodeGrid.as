package com.wg.scene.astar
{
	 ...
	 * 
	 * 在数据上画出场景,场景中可行走不可行走的格子,起点和终点设置;
	 * @author Administrator
	 * 
	 */
	public class NodeGrid
	{
		public var nodeClass:Class = ANode;
...
//		public var bornPos:Array;
		...
		{
			var results:Array = [];
			var n:int = _nodes.length;
			for(var i:int = 0; i < n; i++)
			{
				var rowData:Array = _nodes[i];
				results.push.apply(null, rowData);
			}
			return results;
		}
		
		protected function onCreateANode(x:int, y:int):ANode
		{
...
//		}
		...
		public function findReplacer(toNode:ANode):ANode
		{
			var result:ANode;
			//若终点可移动则根本无需寻找替代点
			if( toNode.walkable )
...
//		{
//			var maxDepth:int = Math.max(_numCols, _numRows);
//			var node:ANode;
//			for(var i:int = 0; i < _numCols; i++)
//			{
//				for(var j:int = 0; j < _numRows; j++)
...
//					else
//					{
//						node.buriedDepth = getNodeBuriedDepth(node, maxDepth);
//					}
//				}
//			}
//		}
