...
package com.wg.ui.scrollBar
{
	...
	public class ScrollBar implements IUI2
	{
		...
		private var _target:DisplayObject;//需要遮罩的mc
		private var _maskTarget:DisplayObject;//遮罩
		private var _scrollBar:Sprite;//滚动条,
		private var _scrollLine:Sprite;//滚动条背景;
		...
		public function set mouseWheel($value:Boolean):void
		{
			...
			if (_mouseWheel && _abled)
				_parentMC.addEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelHandler, false, 0, true );
			else if(_abled)
				_parentMC.removeEventListener(MouseEvent.MOUSE_WHEEL, mouseWheelHandler);
		}
		...
		*/  
		public function ScrollBar($stage:Stage,$target:DisplayObjectContainer, $maskTarget:*, $scrollBar:Sprite, $scrollLine:Sprite, $tween:Number = 5, $elastic:Boolean = false, $lineAbleClick:Boolean = false , $mouseWheel:Boolean = false, $direction:String = "L") 
		{
...
//			_scrollBar.addEventListener(Event.MOUSE_LEAVE,       scrollBarMouseUpHandler, false, 0, true );
		...
		//拖动滑块
		public function makeDragBar(_x:int = 0,_y:int = 0):void
		{
			if(_x&&_y)
			{
				_scrollBar.x = _x;
		...
		private function isHttp(url:String = ""):Boolean
		{
			var tempBln:Boolean;
		...
		public function mouseWheelEnabled(value:Boolean):void {
			...
			//判断是否是浏览器中观看,并解决flash滚动与浏览器滚动的冲突;
			if(isHttp())
			{
				if (!value) {//mousein
		...
		public function dispose():void
		{
			...
			_stage.removeEventListener(MouseEvent.MOUSE_UP,   scrollBarMouseUpHandler, false);
			_stage.removeEventListener(Event.MOUSE_LEAVE,       scrollBarMouseUpHandler, false);
